...
def run_exp(args: Optional[Dict[str, Any]] = None, callbacks: List["TrainerCallback"] = []) -> None:
    ...
    if hasattr(training_args, 'load_best_model_at_end') and training_args.load_best_model_at_end==True:
        callbacks.append(EarlyStoppingCallback(early_stopping_patience=3))

    if os.path.exists(training_args.output_dir):
        rename_dir=training_args.output_dir+'-ep3bk'
        if os.path.exists(rename_dir):
            print(rename_dir)
            os._exit(0)
        os.rename(training_args.output_dir,rename_dir)
    ...
    wandb.init(
        ...
        "architecture": os.path.basename(model_args.model_name_or_path),
    ...
    if finetuning_args.stage == "pt":
        ...
        if hasattr(args,'select_path'):#xy
            data_args.select_path=args.select_path
            data_args.select_mode=args.select_mode
